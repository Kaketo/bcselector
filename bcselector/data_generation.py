...
class MatrixGenerator(_BasicDataGenerator):
    ...
    def _generate_noise(self, sigma, loc=0):
        ...
        noise = np.random.normal(
            loc=loc, scale=noise_sigmas, size=(self.n_rows, self.n_cols)
        )
        return noise
    ...
    def generate(
        ...
        assert discretize_method in [
            "uniform",
            "quantile",
            "kmeans",
        ], "Argument `discretize_method` must be in ['uniform', 'quantile', 'kmeans']"
        assert discretize_bins > 1 and isinstance(
            discretize_bins, int
        ), "Argument `discretize_bins` must be int greater then 1."
...
class DataFrameGenerator(MatrixGenerator):
    ...
    def generate(
        ...
        X, y, costs = super().generate(
            n_rows=n_rows,
            n_basic_cols=n_basic_cols,
            loc=loc,
