...
from bcselector.variable_selection import (
    DiffVariableSelector,
    FractionVariableSelector,
    NoCostVariableSelector,
)
...
class TestFVS_r_paramter(unittest.TestCase):
    def test_criterion_filter_values(self):
        ...
        normalized_costs = list(
            (np.array(costs) - min(costs) + 0.0001) / (max(costs) - min(costs) + 0.0001)
        )
        ...
        self.assertAlmostEqual(
            mutual_information(y, X[:, feature_index]), criterion_value
        )
        ...
        self.assertAlmostEqual(
            ...
            / normalized_costs[feature_index] ** r,
            filter_value,
        )
        ...
        mifs_value = (
            mutual_information(y, X[:, feature_index])
            - mutual_information(X[:, feature_index], X[:, 0])
            - mutual_information(X[:, feature_index], X[:, 3])
        )
        ...
        self.assertAlmostEqual(mifs_value, criterion_value)
        self.assertAlmostEqual(
            (mifs_value + m) / normalized_costs[feature_index] ** r, filter_value
        )
