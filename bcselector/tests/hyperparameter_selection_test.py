...
class TestFVS_r_paramter(unittest.TestCase):
    def test_criterion_filter_values(self):
        ...
        # MIM
        r = 0
        feature_index, filter_value, criterion_value, cost = fraction_find_best_feature(j_criterion_func=mim, r=r, data=X, target_variable=y, possible_variables_index = [0,1,2,3], costs=costs)
        self.assertAlmostEqual(mutual_information(y, X[:,feature_index]), criterion_value)

        r = 5
        feature_index, filter_value, criterion_value, cost = fraction_find_best_feature(j_criterion_func=mim, r=r, data=X, target_variable=y, possible_variables_index = [0,1,2,3], costs=costs)
        self.assertAlmostEqual(mutual_information(y, X[:,feature_index]), criterion_value)
        self.assertAlmostEqual(mutual_information(y, X[:,feature_index])/costs[feature_index]**r, filter_value)
        ...
        self.assertAlmostEqual(mifs_value, criterion_value)
        self.assertAlmostEqual((mifs_value+m)/costs[feature_index]**r, filter_value)