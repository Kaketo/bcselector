...
__all__ = ["mim", "mifs", "mrmr", "jmi", "cife"]
...
def mim(data, target_variable, candidate_variable_index, **kwargs):
    ...
    ), "Argument 'target_variable' must be a numpy matrix"
    assert isinstance(
        candidate_variable_index, int
    ), "Argument 'candidate_variable_index' must be an integer"
    ...
    assert (
        candidate_variable_index < data.shape[1]
    ), "Index 'candidate_variable_index' out of range in 'data'"
...
def mifs(
    data, target_variable, prev_variables_index, candidate_variable_index, **kwargs
):
    """
    ...
    if kwargs.get("beta") is None:
        beta = 1
        warnings.warn(
            "Parameter `beta` not provided, default value of 1 is selected.", Warning
        )
    else:
    ...
    assert isinstance(beta, int) or isinstance(
        beta, float
    ), "Argument 'beta' must be int or float"

    candidate_variable = data[:, candidate_variable_index]
    if len(prev_variables_index) == 0:
        redundancy_sum = 0
    else:
        redundancy_sum = np.apply_along_axis(
            mutual_information,
            axis=0,
            arr=data[:, prev_variables_index],
            vector_2=candidate_variable,
        ).sum()

    return (
        mutual_information(candidate_variable, target_variable) - beta * redundancy_sum
    )
...
def mrmr(
    ...
    candidate_variable = data[:, candidate_variable_index]
    prev_variables_len = (
        1 if len(prev_variables_index) == 0 else len(prev_variables_index)
    )
    ...
    return (
        mutual_information(candidate_variable, target_variable)
        - 1 / prev_variables_len * redundancy_sum
    )
